...
const productTeaserJgQuery = `query productTeaserJg($sku: String!) {
  products(skus: [$sku]) {
    sku
    urlKey
    name
...
export default async function decorate(block) {
  ...
  if (config['background-color']) {
    block.style.setProperty('--ptjg-bg-color', config['background-color']);
  }
  ...
  const { products } = await performCatalogServiceQuery(productTeaserJgQuery, {
    sku: config.sku,
  });
